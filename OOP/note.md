...
- super
    ...
    - super与父类直接没任何实质性关系，但可以通过super调用到父类
...
- 单继承和多继承的优缺点
    ...
    - 多继承：
        ...
        - 缺点：继承关系混乱
    - 菱形继承/钻石继承问题
        - 多个子类继承自同一父类，这些子类又被同一个类继承，于是继承关系图形成一个菱形图谱
        - 关于多继承的MRO
            - MRO就是多级承中，用于保存继承顺序的一个列表
...
- 使用Mixin实现多继承的时候非常小心
    ...
    - 可以方便的组织和维护不同功能组件的划分
    - 可以根据需要任意调整功能类的整合
    - 可以避免创建更多新的类，导致类的继承混乱
# 7 类相关函数
- issubclass检测一个类是否是另一个类的子类
- isinstance:检测一个对象师傅是一个类的实例
...
# 10. 类常用的魔术方法
- 魔术方法就是不需要人为调用的方法，基本在特定是时刻自动触发
- 魔术方法的同一特征，方法名被前后各两个下划线包裹
    - __init__:构造函数 
    - __new__:对象实例化方法，此函数比较 特殊，一般不需要使用
    - __call__:对象当函数使用的时候触发
    - __str__:当对象被当做字符串使用的时候调用
    - __repr__:返回字符串Python3用__str__比较多
- 描述符相关
    _ __set__
    - __get__
    - __delete__
